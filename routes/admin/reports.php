...
$buildAdminReportsData = function (Request $request): array {
    ...
    $detailJoinMonthInput = (string) $request->query('detail_join_month', '');
    $detailExpiredMonthInput = (string) $request->query('detail_expired_month', '');
    $detailJoinMonth = preg_match('/^\d{4}-\d{2}$/', $detailJoinMonthInput) ? Carbon::createFromFormat('Y-m', $detailJoinMonthInput)->startOfMonth() : null;
    $detailExpiredMonth = preg_match('/^\d{4}-\d{2}$/', $detailExpiredMonthInput) ? Carbon::createFromFormat('Y-m', $detailExpiredMonthInput)->startOfMonth() : null;
    ...
    if ($detailJoinMonth) {
        $joinMonthStart = $detailJoinMonth->copy()->startOfMonth();
        $joinMonthEnd = $detailJoinMonth->copy()->endOfMonth();
        $filteredMemberRecords = $filteredMemberRecords->filter(fn($member) => $member->joined_at && $member->joined_at->betweenIncluded($joinMonthStart, $joinMonthEnd));
    }
    if ($detailExpiredMonth) {
        $expiredMonthStart = $detailExpiredMonth->copy()->startOfMonth();
        $expiredMonthEnd = $detailExpiredMonth->copy()->endOfMonth();
        $filteredMemberRecords = $filteredMemberRecords->filter(fn($member) => $member->expires_at && $member->expires_at->betweenIncluded($expiredMonthStart, $expiredMonthEnd));
    }
    ...
    $reportCatalog = collect([
        ...
        return $report;
    })->map(function ($report) use ($memberRecords, $filteredMemberRecords, $checkinRecords, $detailMonth, $detailRangeStart, $detailRangeEnd, $buildDailyRows, $buildMonthlyRows, $formatMoney, $verifiedTransactions, $expenseRecords, $memberPaymentRecords, $nonMemberPaymentRecords, $productSaleRecords, $vitaminProductRecords) {
        ...
        $dummyDetails = match ($report['slug']) {
            ...
            'laporan-keuangan' => [
                ...
                'reportSections' => [
                    'Sumber Pendapatan' => [
                        ...
                        'rows' => [
                            ...
                            ['Pembayaran non-member', $formatMoney($nonMemberPaymentRecords->filter(fn($transaction) => $transaction->transaction_at && $transaction->transaction_at->betweenIncluded($detailRangeStart, $detailRangeEnd))->sum('amount')), 'Daily pass dan visitor'],
                    ...
                    'Pembayaran' => [
                        ...
                        ])->toBase()->merge($nonMemberPaymentRecords->filter(fn($transaction) => $transaction->transaction_at && $transaction->transaction_at->betweenIncluded($detailRangeStart, $detailRangeEnd))->take(5)->map(fn($transaction) => [
                            $transaction->transaction_at?->translatedFormat('d M Y') ?? '-',
                ...
                'overview' => ['Total pemasukan', 'Pembayaran member', 'Pembayaran non-member', 'Riwayat pemasukan', 'Riwayat pengeluaran'],
            ],
            'laporan-kehadiran' => [
                    ...
                    'Rincian Barang Terjual' => [
                        ...
                        'rows' => $stockSales
                            ...
                            ->map(fn($transaction) => [
                                ...
                                $transaction->invoice ?: '-',
                                $transaction->customer_name ?: ($transaction->member?->full_name ?? 'Guest'),
    ...
    return [
        ...
        'detailJoinMonth'         => $detailJoinMonth?->format('Y-m'),
        'detailExpiredMonth'      => $detailExpiredMonth?->format('Y-m'),
        'detailJoinMonthLabel'    => $detailJoinMonth?->translatedFormat('F Y') ?: 'Semua',
        'detailExpiredMonthLabel' => $detailExpiredMonth?->translatedFormat('F Y') ?: 'Semua',
